                    ...
                    <div className="px-7 pb-6 pt-4">
                        <ul>
                            <li className="block mt-3">
                                <button onClick={() => setSelectedCategory(null)} className={`text-gray-500 w-full px-3 py-2 rounded-2xl flex justify-between hover:active ${!selectedCategory && `active`}`}>
                                    الكل
                                </button>
                            </li>
                            {categories.map((category) => {return (
                                <li key={category.id} className="block mt-3">
                                    <button onClick={() => setSelectedCategory(category.id)} className={`text-gray-500 w-full px-3 py-2 rounded-2xl flex justify-between hover:active ${selectedCategory === category.id && `active`}`}>
                                        {category.title}
